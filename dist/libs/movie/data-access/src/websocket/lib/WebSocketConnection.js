...
WebSocketConnection.prototype.sendUTF = function(data, cb) {
    ...
    frame.opcode = 1; // WebSocketOpcode.TEXT_FRAME
...
WebSocketConnection.prototype.sendBytes = function(data, cb) {
    ...
    var frame = new WebSocketFrame(this.maskBytes, this.frameHeader, this.config);
    frame.opcode = 2; // WebSocketOpcode.BINARY_FRAME
    frame.binaryPayload = data;
    this.fragmentAndSend(frame, cb);
};
WebSocketConnection.prototype.ping = function(data) {
    this._debug('ping');
    var frame = new WebSocketFrame(this.maskBytes, this.frameHeader, this.config);
    frame.opcode = 9; // WebSocketOpcode.PING
    frame.fin = true;
    if (data) {
        if (!Buffer.isBuffer(data)) {
            data = bufferFromString(data.toString(), 'utf8');
...
WebSocketConnection.prototype.pong = function(binaryPayload) {
    this._debug('pong');
    var frame = new WebSocketFrame(this.maskBytes, this.frameHeader, this.config);
    frame.opcode = 10; // WebSocketOpcode.PONG
...
WebSocketConnection.prototype.sendCloseFrame = function(reasonCode, description, cb) {
    ...
    var frame = new WebSocketFrame(this.maskBytes, this.frameHeader, this.config);
    frame.fin = true;
    frame.opcode = 8; // WebSocketOpcode.CONNECTION_CLOSE
...
function instrumentSocketForDebugging(connection, socket) {
    ...
    for(var key1 in socket){
        if ('function' !== typeof socket[key1]) {
            continue;
        }
        if ([
            'emit'
