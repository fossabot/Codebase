...
const TMP_FOLDER_BASE = path.join(os.tmpdir(), TMP_FOLDER_NAME);
...
function getPaths(channelName) {
    if (!getPathsCache.has(channelName)) {
        ...
        const channelPathBase = path.join(TMP_FOLDER_BASE, channelFolder);
        const folderPathReaders = path.join(channelPathBase, 'rdrs');
        const folderPathMessages = path.join(channelPathBase, 'messages');
        const ret = {
...
function socketPath(channelName, readerUuid, paths) {
    paths = paths || getPaths(channelName);
    const socketPath1 = path.join(paths.readers, readerUuid + '.s');
    return cleanPipeName(socketPath1);
}
function socketInfoPath(channelName, readerUuid, paths) {
    paths = paths || getPaths(channelName);
    const socketPath2 = path.join(paths.readers, readerUuid + '.json');
    return socketPath2;
}
/**
...
async function connectionError(originalError) {
    ...
    const text = 'BroadcastChannel.create(): error: ' + 'This might happen if you have created to many channels, ' + 'like when you use BroadcastChannel in unit-tests.' + 'Try using BroadcastChannel.clearNodeFolder() to clear the tmp-folder before each test.' + 'See https://github.com/pubkey/broadcast-channel#clear-tmp-folder';
...
async function openClientConnection(channelName, readerUuid) {
    ...
    return new Promise((res, rej)=>{
        client.connect(pathToSocket, ()=>res(client)
        );
 ...
 */ function writeMessage(channelName, readerUuid, messageJson, paths) {
    ...
    const msgPath = path.join(paths.messages, fileName);
    return writeFile(msgPath, JSON.stringify(writeObject)).then(()=>{
        return {
            time,
            uuid: readerUuid,
            token,
...
async function messagePath(channelName, time, token, writerUuid) {
    ...
    const msgPath = path.join(getPaths(channelName).messages, fileName);
    return msgPath;
}
...
function readMessage(messageObj) {
    return readFile(messageObj.path, 'utf8').then((content)=>JSON.parse(content)
    );
}
async function cleanOldMessages(messageObjects, ttl) {
    const olderThen = Date.now() - ttl;
    await Promise.all(messageObjects.filter((obj)=>obj.time / 1000 < olderThen
 ...
 */ async function create(channelName, options = {}) {
    ...
    const [socketEE, infoFilePath] = await Promise.all([
    ...
    socketEE.emitter.on('data', (data)=>{
        ...
        singleOnes.filter((single)=>single !== ''
        ).forEach((single)=>{
            try {
 ...
 * run this
 */ async function handleMessagePing(state, msgObj1) {
    /**
    ...
    const useMessages = messages.filter((msgObj)=>_filterMessage(msgObj, state)
    ...
    // read contents
    await Promise.all(useMessages.map((msgObj)=>readMessage(msgObj).then((content)=>msgObj.content = content
        )
    ));
 ...
 */ function refreshReaderClients(channelState) {
    return getReadersUuids(channelState.channelName, channelState.paths).then((otherReaders)=>{
        // remove subscriptions to closed readers
        Object.keys(channelState.otherReaderClients).filter((readerUuid)=>!otherReaders.includes(readerUuid)
        ...
        // add new readers
        return Promise.all(otherReaders.filter((readerUuid)=>readerUuid !== channelState.uuid
        ) // not own
        .filter((readerUuid)=>!channelState.otherReaderClients[readerUuid]
 ...
 */ function postMessage(channelState, messageJson) {
    const writePromise = writeMessage(channelState.channelName, channelState.uuid, messageJson, channelState.paths);
    channelState.writeBlockPromise = channelState.writeBlockPromise.then(async ()=>{
        ...
        const writeToReadersPromise = Promise.all(Object.values(channelState.otherReaderClients).filter((client)=>client.writable
         ...
         */ if (randomInt(0, 20) === 0) {
            /* await */ getAllMessages(channelState.channelName, channelState.paths).then((allMessages)=>cleanOldMessages(allMessages, channelState.options.node.ttl)
            );
        }
 ...
 */ function emitOverFastPath(state, msgObj, messageJson) {
    ...
    others.filter((otherState)=>_filterMessage(checkObj, otherState)
    ).forEach((otherState)=>{
 ...
 */ function close(channelState) {
    ...
    return new Promise((res)=>{
        ...
        Object.values(channelState.otherReaderClients).forEach((client)=>client.destroy()
        );
