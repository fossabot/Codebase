"use strict";
var assert = require("chai").assert, sinon = require("sinon"), identity = require("../../function/identity"), finallyMethod = require("../../thenable_/finally");
...
describe("thenable_/finally", function() {
    ...
    describe("Successful on rejected", function() {
        ...
        it("Return promise should fulfill with original result", function() {
            return Promise.all([
                input["catch"](identity),
                result["catch"](identity)
            ]).then(function(results) {
                assert.equal(results[0], results[1]);
            });
        });
