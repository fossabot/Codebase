...
// Strategies
(function() {
    for(var i = 0; i <= 255; ++i)cacheCounter[i] = 0;
})();
fromCache = function(nextStep, cp, needFeature) {
    var ret = cache[cp];
    if (!ret) {
        ret = nextStep(cp, needFeature);
        if (Boolean(ret.feature) && ++cacheCounter[cp >> 8 & 255] > CACHE_THRESHOLD) {
            cache[cp] = ret;
        }
    }
    return ret;
...
fromData = function(next, cp) {
    var hash = cp & 65280, dunit = UChar.udata[hash] || {}, feature = dunit[cp];
...
UChar.prototype.toString = function() {
    ...
    num = this.codepoint - 65536;
    return String.fromCharCode(floor(num / 1024) + 55296, num % 1024 + 56320);
};
