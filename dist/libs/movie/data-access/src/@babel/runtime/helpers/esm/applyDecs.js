...
function createMetadataMethodsForProperty(metadataMap, kind, property) {
    return {
        getMetadata: function getMetadata(key) {
            if ("symbol" != (0, _typeofJs).default(key)) throw new TypeError("Metadata keys must be symbols, received: " + key);
...
function assertValidReturnValue(kind, value) {
    var type = (0, _typeofJs).default(value);
    if (1 === kind) {
...
function applyMemberDec(ret, base, decInfo, name, kind, isStatic, isPrivate, metadataMap, initializers) {
    var desc, initializer, value1, decs = decInfo[0];
    isPrivate ? desc = 0 === kind || 1 === kind ? {
        ...
        value: decInfo[3]
    } : 0 !== kind && (desc = Object.getOwnPropertyDescriptor(base, name)), 1 === kind ? value1 = {
        get: desc.get,
        set: desc.set
    } : 2 === kind ? value1 = desc.value : 3 === kind ? value1 = desc.get : 4 === kind && (value1 = desc.set);
    var newValue, get, set, ctx = memberDecCtx(base, name, desc, metadataMap, initializers, kind, isStatic, isPrivate);
    if ("function" == typeof decs) void 0 !== (newValue = decs(value1, ctx)) && (assertValidReturnValue(kind, newValue), 0 === kind ? initializer = newValue : 1 === kind ? (initializer = newValue.initializer, get = newValue.get || value1.get, set = newValue.set || value1.set, value1 = {
        get: get,
        set: set
    }) : value1 = newValue);
    else for(var i1 = decs.length - 1; i1 >= 0; i1--){
        var newInit;
        if (void 0 !== (newValue = (0, decs[i1])(value1, ctx))) assertValidReturnValue(kind, newValue), 0 === kind ? newInit = newValue : 1 === kind ? (newInit = newValue.initializer, get = newValue.get || value1.get, set = newValue.set || value1.set, value1 = {
            get: get,
            set: set
        }) : value1 = newValue, void 0 !== newInit && (void 0 === initializer ? initializer = newInit : "function" == typeof initializer ? initializer = [
            initializer,
            newInit
    ...
    0 !== kind && (1 === kind ? (desc.get = value1.get, desc.set = value1.set) : 2 === kind ? desc.value = value1 : 3 === kind ? desc.get = value1 : 4 === kind && (desc.set = value1), isPrivate ? 1 === kind ? (ret.push(function(instance, args) {
        return value1.get.call(instance, args);
    }), ret.push(function(instance, args) {
        return value1.set.call(instance, args);
    })) : 2 === kind ? ret.push(value1) : ret.push(function(instance, args) {
        return value1.call(instance, args);
...
function applyMemberDecs(ret, Class, protoMetadataMap, staticMetadataMap, decInfos) {
    for(var protoInitializers = [], staticInitializers = [], existingProtoNonFields = new Map(), existingStaticNonFields = new Map(), i = 0; i < decInfos.length; i++){
        ...
        if (Array.isArray(decInfo)) {
            var base, metadataMap, initializers, kind = decInfo[1], name = decInfo[2], isPrivate = decInfo.length > 3, isStatic = kind >= 5;
...
function applyClassDecs(ret, targetClass, metadataMap, classDecs) {
    ...
    }, createMetadataMethodsForProperty(metadataMap, 0, name)), i2 = classDecs.length - 1; i2 >= 0; i2--){
        newClass = classDecs[i2](newClass, ctx) || newClass;
    }
